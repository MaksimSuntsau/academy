...
import by.academy.homework.hw3.validation.BelarussianPhoneValidator;
import by.academy.homework.hw3.validation.EmailValidator;
...
//		Product[] products = deal.getProducts();
	...
	private static Person inputPerson() throws ParseException {
		...
		System.out.println("Номер " + number + (v.isValid(number) == true ? " подтверждён" : " неподтверждён"));

		String email = input("Почта");
		EmailValidator em = new EmailValidator();

		System.out.println("Email " + email + (em.isValid(email) == true ? " подтверждён" : " неподтверждён"));
